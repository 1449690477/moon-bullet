...
describe('dream mode level one: Fallen Radiance Sanctuary', () => {
  ...
  it('registers a separate deterministic dream run without changing chapter mode', () => {
    ...
    expect(dream.levelSpec()).toMatchObject({
      ...
      seed: 7130101,
      waveCount: 10,
      mobTargetSeconds: 300,
      bossTargetSeconds: 90,
      bossKey: 'seraph',
    });
  ...
  it('defines exactly three larger high-HP elites for every wave', () => {
    const mobs = dream.mobSpec();
    expect(mobs).toEqual([
      expect.objectContaining({ key: 'dream_seraph_wing', name: '曦翼星使', sprite: 'drone', hp: 7200, size: 118, r: 29 }),
      expect.objectContaining({ key: 'dream_seraph_choir', name: '辉歌咏者', sprite: 'nun', hp: 9600, size: 126, r: 31 }),
      expect.objectContaining({ key: 'dream_seraph_guard', name: '圣盾执刑者', sprite: 'holyshield', hp: 13200, size: 136, r: 35 }),
    ]);
    expect(mobs.map((mob) => mob.role)).toEqual(['wing', 'choir', 'guard']);
  });

  it('builds ten seeded 2/2/2 waves with the documented HP curve', () => {
    const expectedMultipliers = [1, 1.04, 1.08, 1.12, 1.16, 1.2, 1.25, 1.3, 1.36, 1.42];
    ...
    for (let index = 0; index < waves.length; index += 1) {
      expect(waves[index]).toMatchObject({ count: 6, composition: [2, 2, 2], batches: [3, 3], batchDelay: 6 });
  ...
  it('uses a shared conductor with bounded complete pattern groups', () => {
    ...
    expect(dream.poolReuseForTest()).toMatchObject({ capacity: 128, peakActive: expect.any(Number), overflowed: false, reused: true });
    expect(dream.poolReuseForTest().peakActive).toBeLessThanOrEqual(96);
  });
  ...
  it('keeps the logical barrage identical in all four quality modes', () => {
    const samples = ['high', 'medium', 'low', 'ultra'].map((quality) => dream.bulletSnapshotForTest({
      wave: 9,
      elapsed: 12.5,
      quality,
    }));
    expect(samples.map((sample) => sample.logicalCount)).toEqual([samples[0].logicalCount, samples[0].logicalCount, samples[0].logicalCount, samples[0].logicalCount]);
    expect(samples.map((sample) => sample.logicHash)).toEqual([samples[0].logicHash, samples[0].logicHash, samples[0].logicHash, samples[0].logicHash]);
  ...
  it('defines the separate five-phase 96k seraph encounter', () => {
    ...
    expect(boss).toMatchObject({
      ...
      hp: 96000,
      creditCap: 7680,
      creditRechargePerSecond: 1067,
      targetSeconds: [85, 110],
  ...
  it('uses isolated one-time-token backend routes and a read-only public score table', () => {
    ...
    expect(edgeSource).toContain('"dream-01-seraph", { clearVersion: "dream-01-v1", seed: 7130101 }');

-- 梦境模式第二关排行榜契约。
-- 依赖 09_dream_leaderboard.sql 与 10_dream_v2_balance.sql；保留第一关 V1/V2 全部成绩。
...
begin
  if to_regclass('public.dream_leaderboard_runs') is null
    ...
    raise exception 'Run leaderboard-security/09_dream_leaderboard.sql and 10_dream_v2_balance.sql before 11_dream_level_two.sql';
  end if;
end
$$;
...
alter table public.dream_leaderboard_runs
  ...
  check (stage_id in ('dream-01-seraph', 'dream-02-zero-compile')) not valid;
alter table public.dream_leaderboard_runs
  add constraint dream_runs_version_check
  check (clear_version in ('dream-01-v1', 'dream-01-v2', 'dream-02-v1')) not valid;
alter table public.dream_leaderboard_runs
  add constraint dream_runs_seed_check
  check (seed in (7130101, 7130202)) not valid;
alter table public.dream_leaderboard_runs
...
-- 保留旧 schema RPC 返回值供第一关客户端兼容，新增多关卡契约探针。
...
comment on function public.dream_leaderboard_stage_contract() is
  'Dream leaderboard stage whitelist contract: dream-01 V1/V2 plus dream-02 V1.';
